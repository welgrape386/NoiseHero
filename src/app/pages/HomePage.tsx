...
const mockHistory = [
  { id: 1, time: '어제 23:14', db: 58, type: '직접충격', sub: 'Lmax 71', over: true },
  { id: 2, time: '어제 19:32', db: 36, type: '공기전달', sub: 'Leq 36', over: false },
  { id: 3, time: '5/1 14:05', db: 61, type: '직접충격', sub: 'Lmax 74', over: true },
];
...
export function HomePage() {
  ...
  const user = JSON.parse(localStorage.getItem('noise_user') || '{}');
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ fontSize: 11, padding: '4px 12px', borderRadius: 999, border: '1px solid #cfd6ea', color: '#7A8AB8' }}>주간</div>
        ...
        <GlassCard style={{ marginTop: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginTop: 12, gap: 10 }}>
            <div style={{
              ...
              background: 'rgba(217,48,37,0.12)', color: '#C0271E',
              border: '1px solid rgba(217,48,37,0.25)',
            }}>
              <div style={{ width: 6, height: 6, background: '#D93025', borderRadius: '50%' }} />
              기준 초과 (Leq +19 dB)
            </div>
            <div style={{ fontSize: 11, color: '#9AA6C0' }}>직접충격 · 주간</div>
          </div>

          <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
            {[
              { label: 'Leq 평균', val: 58, pct: 80 },
              { label: 'Lmax 최고', val: 71, pct: 92 },
            ].map(item => (
              <div
                key={item.label}
                style={{
              ...
              >
                ...
                <div style={{ height: 6, background: '#e5e9f5', borderRadius: 6, marginTop: 8, overflow: 'hidden' }}>
                  <div style={{ width: `${item.pct}%`, height: '100%', borderRadius: 6, background: 'linear-gradient(90deg, #ff6b6b, #D93025)' }} />
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Weekly Stats */}
        <div style={{ marginTop: 26, marginBottom: 12, fontSize: 14, fontWeight: 600, color: '#8C98B8' }}>이번 주 통계</div>
          ...
          {[
            ...
            <GlassCardSm key={i} style={{ padding: 16, position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 20, fontWeight: 700, color: stat.valColor, lineHeight: 1 }}>
                {stat.val}
                {stat.unit && <span style={{ fontSize: 12, fontWeight: 500, color: '#9AA6C0', marginLeft: 2 }}>{stat.unit}</span>}
              </div>
        ...
        <GlassCardSm style={{ padding: '8px 16px' }}>
          {mockHistory.map((item, idx) => (
            ...
            >
              ...
              <div style={{ fontSize: 11, color: '#9AA6C0', width: 60, flexShrink: 0 }}>{item.time}</div>
              <div style={{ flex: 1 }}>
