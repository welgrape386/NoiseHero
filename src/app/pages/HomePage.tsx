...
export function HomePage() {
  ...
  const timeLabel = night ? '?ºÍ∞Ñ' : 'Ï£ºÍ∞Ñ';
  ...
  const recent = history; // APIÍ∞Ä ?¥Î? ÏµúÍ∑º?úÏúºÎ°?Î∞òÌôò
  const recentWeek = recent.filter(i => {
    // measured_at Í∏∞Î∞ò?ºÎ°ú ?ÑÌÑ∞?òÍ≥† ?∂Ï?Îß?HistoryItem?Ä time(Î¨∏Ïûê??Îß??àÏùå
    // ???ÑÏ≤¥ ?¥Î†• ?¨Ïö© (Í∞ÑÏÜå??
    return true;
  });
  ...
  const avgDb = totalCount > 0
    ...
    : 0;
  const nightOverCount = recentWeek.filter(i => i.over && i.period === '?ºÍ∞Ñ').length;

  // ÏµúÍ∑º Ï∏°Ï†ïÍ∞?(Ï≤?Î≤àÏß∏ ?¥Î†•)
  const latest = history[0];

  // ÏµúÍ∑º 3Í∞úÎßå ?àÏóê ?úÏãú
    ...
    <div style={{ position: 'relative', width: '100%', height: '100vh', background: '#F0F2FA', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, flex: 1, overflowY: 'auto', padding: '20px 20px 100px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 22, fontWeight: 700, color: '#0A1866' }}>
              ?åÏùå<span style={{ color: '#1A3BDB' }}>ON</span>
            </div>
        ...
        <GlassCard style={{ marginTop: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#8C98B8', marginBottom: 8 }}>ÏµúÍ∑º Ï∏°Ï†ïÍ∞?/div>
          ...
          {latest ? (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', marginTop: 12, gap: 10 }}>
                <div style={{
                  ...
                  {latest.over
                    ? `Í∏∞Ï? Ï¥àÍ≥º (+${Math.round((latest.db - (latest.leq_standard ?? 39)) * 10) / 10} dB)`
                    : '?ïÏÉÅ Î≤îÏúÑ'}
                </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
                {[
                  { label: 'Leq ?âÍ∑†', val: latest.db, pct: Math.min(latest.db, 100) },
                  { label: 'Lmax ÏµúÍ≥†', val: latest.lmax, pct: Math.min(latest.lmax, 100) },
                ].map(item => (
            ...
            <div style={{ paddingTop: 12, paddingBottom: 8, fontSize: 13, color: '#9AA6C0', textAlign: 'center' }}>
              ?ÑÏßÅ Ï∏°Ï†ï ?¥Î†•???ÜÏäµ?àÎã§.
            </div>
          )}
        </GlassCard>

        {/* Weekly Stats */}
        <div style={{ marginTop: 26, marginBottom: 12, fontSize: 14, fontWeight: 600, color: '#8C98B8' }}>?ÑÏ†Å ?µÍ≥Ñ</div>
        ...
        {/* Recent History */}
        <div style={{ marginTop: 26, marginBottom: 12, fontSize: 14, fontWeight: 600, color: '#8C98B8' }}>ÏµúÍ∑º ?¥Î†•</div>
        {recentThree.length > 0 ? (
          ...
          <GlassCardSm style={{ padding: '20px 16px', textAlign: 'center' }}>
            <div style={{ fontSize: 13, color: '#9AA6C0' }}>?ÑÏßÅ Ï∏°Ï†ï ?¥Î†•???ÜÏäµ?àÎã§.</div>
            <div style={{ fontSize: 11, color: '#B0B8D0', marginTop: 4 }}>Ï∏°Ï†ï???úÏûë?¥Î≥¥?∏Ïöî!</div>
          </GlassCardSm>
        )}
